...
class ProfileController extends Controller
{
    ...
    {
        $request->validate([
            'banner' => 'nullable|image|mimes:jpeg,png,gif,webp|max:5120', // 5MB
        ...
        if ($request->hasFile('banner')) {
            try {
                ...
                // Process and store the banner image
                $file = $request->file('banner');
                $filename = 'banner_' . $user->id . '_' . time() . '.webp';
                
                // Create directory if it doesn't exist
                Storage::disk('public')->makeDirectory('banners', 0755, true);

                // Read the image file and convert to WebP
                $manager = new ImageManager(new Driver());
                $image = $manager->read($file->getRealPath());
                
                // Resize banner to standard dimensions (1200x400)
                $image = $image->scale(1200, 400);

                // Convert to WebP and store
                $webpContent = $image->toWebp(85);
                Storage::disk('public')->put('banners/' . $filename, $webpContent);

                // Update user banner path
                $user->banner = 'storage/banners/' . $filename;
            } catch (\Exception $e) {
